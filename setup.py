...
DESCRIPTION = "Simple Webserver."                                               # A short description of the application
...
REAUIRED_LIBS = []                                                              #A python list of required libs (optionally including versions)
...
class CustomInstallCommand(install):
    ...
    def _createCmd(self, moduleName, mainModule, path="/usr/local/bin"):
        ...
        if os.path.isdir(path):
            ...
            tmpFile = fd.name
            if PYTHON_VERSION == 3:
                fd.write( str.encode("#!/usr/bin/env python3\n") )
            else:
                fd.write( str.encode("#!/usr/bin/env python2\n") )
            fd.write( str.encode("from %s import %s\n" % (moduleName, mainModule)) )
            fd.write( str.encode("%s.main()\n" % (mainModule)) )
            fd.close()
